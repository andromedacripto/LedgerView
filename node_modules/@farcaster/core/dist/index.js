...
__export(src_exports, {
  ...
  DbMerkleTrieMetadata: () => DbMerkleTrieMetadata,
  DbStats: () => DbStats,
  ...
  makeLendStorage: () => makeLendStorage,
  makeLendStorageData: () => makeLendStorageData,
...
var validateMessageData = async (data, publicClients = defaultPublicClients) => {
  ...
  if (validType.value === 1 /* CAST_ADD */ && !!data.castAddBody) {
    ...
    if (data.timestamp < farcasterTime.value - 60 * 10) {
      bodyResult = (0, import_neverthrow6.err)(new HubError("bad_request.validation_failure", "timestamp more than 10 mins in the past"));
    } else {
...
var validateLendStorageBody = (body) => {
  if (body.numUnits > 5e3) {
    return (0, import_neverthrow6.err)(new HubError("bad_request.validation_failure", "num storage units too large"));
  }
  return (0, import_neverthrow6.ok)(body);
};
...
var validateUserDataAddBody = (body) => {
  ...
  switch (type) {
    ...
    case 13 /* PROFILE_TOKEN */: {
      if (value !== "") {
        if (valueBytes.length > 256) {
          return (0, import_neverthrow6.err)(new HubError("bad_request.validation_failure", "profile token value > 256"));
        }
        const validatedProfileToken = validateProfileToken(value);
        if (validatedProfileToken.isErr()) {
          return (0, import_neverthrow6.err)(validatedProfileToken.error);
        }
      }
      break;
    }
...
var validateProfileToken = (token) => {
  ...
  if (!caip19Regex.test(token)) {
    return (0, import_neverthrow6.err)(new HubError("bad_request.validation_failure", "profile token is not a valid CAIP-19 token"));
  }
  return (0, import_neverthrow6.ok)(token);
};
